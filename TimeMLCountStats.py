...
# with open('datasets/te3/te3-platinum_v2.txt', 'r') as reader:
    for line in reader:
        if line.startswith("#"):
            ...
            # for i, head_id in enumerate(rel_head_id):
            #     if int(head_id) > max_index[sentence_index] or int(head_id) < -1:
            #         rel_head_id_int.append(head_id)
            #         rel.append(relation[i])

            for i, head_id in enumerate(rel_head_id):

                # adjacent inter-sentences plus
                if int(head_id) > max_index[sentence_index]:
                    if int(head_id) < (max_index[sentence_index]+max_index[sentence_index+1]+1):
                ...
                # # adjacent inter-sentences minus
                # if int(head_id) < -1:
                #     if abs(int(head_id)) < (max_index[sentence_index-1]+1):
                #         if relation[i] in ["'BEFORE'", "'AFTER'", "'INCLUDES'", "'IS_INCLUDED'", "'DURING'",
                #                            "'SIMULTANEOUS'", "'IAFTER'", "'IBEFORE'", "'IDENTITY'", "'BEGINS'",
                #                            "'ENDS'", "'BEGUN_BY'", "'ENDED_BY'", "'DURING_INV'"]:
                #             rel_head_id_int.append(head_id)
...
with open('datasets/TBAQ-cleaned/timebank_stats.txt', 'w') as w:
# with open('datasets/TBAQ-cleaned/aquaint_stats.txt', 'w') as w:
# with open('datasets/te3/te3-platinum_stats.txt', 'w') as w:
