...
from timebank_v1_modeling_bert import BertForTemporalMultitask
...
class Trainer:
    ...
    def merge_datasets(self) -> DataLoader:
        ...
        data_loader = DataLoader(
            concat_dataset,
            batch_size=self.args.train_batch_size,
            sampler=BatchSchedulerSampler(dataset=concat_dataset, batch_size=self.args.train_batch_size),
            collate_fn=self.data_collator.collate_batch,
        )
    ...
    def evaluate(
        ...
        preds_tensor = torch.from_numpy(output.predictions)
        preds_argmax = torch.argmax(preds_tensor, dim=1)

        output_pred_file = os.path.join(
            self.args.output_dir, f"pred_results.txt"
        )
    ...
    def evaluate_multi(
        ...
        # output_eval = self._prediction_loop_timebank(eval_dataloader, description="Evaluation")
