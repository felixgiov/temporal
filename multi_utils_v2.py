...
def match_ner_label(bpe_x, y, tokenizer=None):
    bpe_y = y.copy()
    for i in range(len(bpe_x)):
        if isinstance(tokenizer, BertTokenizer):
            ...
            if i == 0:
                continue
            if not bpe_x[i].startswith('Ġ'):
                if '-' in bpe_y[i - 1]:
                    bpe_y.insert(i, 'I' + bpe_y[i - 1][1:])
                else:
                    bpe_y.insert(i, bpe_y[i - 1])
        else:
            raise ValueError("Unknown type of tokenizer")
    assert len(bpe_x) == len(bpe_y)
    return bpe_y


def match_mod_label(bpe_x, y, default_lab='_', tokenizer=None):
    bpe_y = y.copy()
    for i in range(len(bpe_x)):
        if isinstance(tokenizer, BertTokenizer):
            if bpe_x[i].startswith('##'):
...
def explore_unk(bpe_x, ori_x, unk_tok='[UNK]'):
    ix_count = 0
    deunk_bpe_x = []
...
#
#                             assert len(toks) == len(labs) == len(modality_labs) == len(cert_labs) == len(
#                                 ttype_labs) == len(state_labs)
#
...
#                                 fo.write("{}\t{}\t{}\t{}\t{}\n".format(i, t, l, r, h))
#                         else:
#                             for i, t, l, ml, r, h in zip(tok_ids, toks, labs, modality_labs, tok_rel_list,
#                                                          tok_head_list):
#                                 fo.write("{}\t{}\t{}\t{}\t{}\t{}\n".format(i, t, l, ml, r, h))
#
...
def extract_cert_from_conll(conll_file, tokenizer, attrib_lab2ix, device, max_ner_num=8, attrib='cert',
                            test_mode=False):
...
def measure_modality_fscore(gold_tags, pred_tags):
    ...
    for k, v in counts.items():
        ...
        print('Modality Tag: %s, precision: %.2f%%, recall: %.2f%%, f1: %.2f' % (
...
def extract_entity_ids_from_conll_sent(conll_sent, col_names):
    ...
    for sent_id, toks in enumerate(conll_sent):
        bio, tag = toks[col_names.index("ner")].split('-', 1) if len(
            toks[col_names.index("ner")].split('-', 1)) > 1 else ('O', 'N')
...
def convert_rels_to_tensors(ner_toks, ner_labs, rels,
    ...
    for sent_toks, sent_labs, sent_rels in zip(ner_toks, ner_labs, rels):
        ...
        sbw_sent_labs = match_ner_label(sbw_sent_toks, sent_labs, tokenizer)
...
def align_ner_to_sbw(ori_y, tok_to_sbw):
    assert len(ori_y) == len(tok_to_sbw)
    sbw_y = []
    for tok_y, sbw_ids in zip(ori_y, tok_to_sbw):
        if '-' in tok_y:
            sbw_y.append(tok_y)
            sbw_y.extend([f"I-{tok_y.split('-', 1)[1]}"] * (len(sbw_ids) - 1))
        else:
            sbw_y.extend([tok_y] * len(sbw_ids))
    assert len(sbw_y) == sum([len(sbw_ids) for sbw_ids in tok_to_sbw])
    return sbw_y


def align_mod_to_sbw(ori_y, tok_to_sbw, default_mod='_'):
    assert len(ori_y) == len(tok_to_sbw)
    sbw_y = []
    for tok_y, sbw_ids in zip(ori_y, tok_to_sbw):
        sbw_y.extend([default_mod] * len(sbw_ids))
        if tok_y != '_':
            sbw_y[-1] = tok_y
    assert len(sbw_y) == sum([len(sbw_ids) for sbw_ids in tok_to_sbw])
    return sbw_y
...
def convert_rels_to_mhs_v3(
    ...
    for sent_id, (sent_tok, sent_ner, sent_mod, sent_rel) in enumerate(zip(ner_toks, ners, mods, rels)):
        ...
        # wrapping data with 'cls_tok' and 'sep_tok'
        if deunk:
        ...
        cls_sbw_sent_tok = [cls_tok] + sbw_sent_tok + [sep_tok]
        cls_tok_to_sbw_ids = align_sbw_ids(cls_sbw_sent_tok)
        assert len(cls_tok_to_sbw_ids) == (len(sent_tok) + 2)

        cls_sbw_sent_mask = [1] * len(cls_sbw_sent_tok)

        cls_sent_ner = ['O'] + sent_ner + ['O']
        cls_sent_mod = ['_'] + sent_mod + ['_']
        cls_sbw_sent_ner = align_ner_to_sbw(cls_sent_ner, cls_tok_to_sbw_ids)
        cls_sbw_sent_mod = align_mod_to_sbw(cls_sent_mod, cls_tok_to_sbw_ids)

        if verbose:
            print(f"sent_id: {sent_id}/{doc_num}")
            print(sent_tok)
            print([f"{index}: {tok}" for index, tok in enumerate(cls_sbw_sent_tok)])
            print([f"{index}: {ner}" for index, ner in enumerate(cls_sbw_sent_ner)])
            print([f"{index}: {mod}" for index, mod in enumerate(cls_sbw_sent_mod)])
        ...
        for tail_ids, tail_lab, head_ids, head_lab, rel_lab in sent_rel:
            tail_last_id = cls_tok_to_sbw_ids[int(tail_ids[-1]) + 1][-1]  # with the begining [CLS] + 1
            head_last_id = cls_tok_to_sbw_ids[int(head_ids[-1]) + 1][-1]  # with the begining [CLS] + 1
            rel_item = (tail_last_id, head_last_id, rel_lab)
            sent_rel_tuples.append(rel_item)
            sbw_tail_tok = [cls_sbw_sent_tok[a_i] for o_i in tail_ids for a_i in cls_tok_to_sbw_ids[int(o_i) + 1]]
            sbw_head_tok = [cls_sbw_sent_tok[a_i] for o_i in head_ids for a_i in cls_tok_to_sbw_ids[int(o_i) + 1]]
            spo_item = {'subject': sbw_tail_tok, 'predicate': rel_lab, 'object': sbw_head_tok}
            sent_spo.append(spo_item)
            if verbose:
                print(rel_item)
                print(["{}: {}".format(ix, a_i) for ix, a_i in enumerate(cls_tok_to_sbw_ids)])
                print((tail_ids, rel_lab, head_ids))
                print(spo_item)
            rel_count += 1
        if verbose:
